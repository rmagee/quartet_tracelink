...
class TracelinkOutputStep(EPCPyYesOutputStep):
    ...
    def generate_sbdh(self, header_version = '1.0', sender_sgln = None,
                      receiver_sgln = None, doc_id_standard='EPCGlobal', doc_id_type_version='1.0',
                      doc_id_instance_identifier=None, doc_id_type='Events', creation_date_and_time=None):
        '''
        Slap in an SBDH.
        '''
        if sender_sgln and receiver_sgln:
            sender = sbdh.Partner(sbdh.PartnerType.SENDER, sbdh.PartnerIdentification('SGLN', sender_sgln))
            receiver = sbdh.Partner(sbdh.PartnerType.RECEIVER, sbdh.PartnerIdentification('SGLN', receiver_sgln))
        ...
        if len(all_events) > 0:
            for event in all_events:
                if event.source_list and event.destination_list:
                    ...
                    sbdh_out = self.generate_sbdh(sender_sgln=source_sgln, receiver_sgln=destination_sgln)
                    break
            epcis_document = template_events.EPCISEventListDocument(all_events, sbdh_out, template=env.get_template('quartet_tracelink/tracelink_epcis_events_document.xml'))
